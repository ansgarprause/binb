...
const { usersPrefix } = require('../redis-config');
...
exports.unban = function(ip, spark, callback) {
  ...
  usersClient.hget(['user:' + issuedby, 'role'], function(err, role) {
    if (err) {
    ...
    if (ip !== 'list') {
      return usersClient.del('ban:' + ip);
    }

    // List all banned players
    usersClient.keys([usersPrefix + 'ban:*'], function(err, replies) {
      if (err) {
      ...
      replies.forEach(function(key) {
        ...
        usersClient.get([key], function(err, reply) {
          if (err) {
            return console.error(err.message);
          }
